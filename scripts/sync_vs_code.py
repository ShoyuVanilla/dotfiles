...
def apply_settings(src_path, dest_path, platform_name):
    common = json.load(
        open(src_path.joinpath("settings.common.json"), encoding="utf-8")
    )
...
def apply_keybindings(src_path, dest_path, platform_name):
    src = json.load(
        open(src_path.joinpath("keybindings.common.json"), encoding="utf-8")
    )
...
def install_extensions(src_path, remove_undefined):
    extensions_local = set(
        subprocess.check_output(["code", "--list-extensions"], shell=True)
        .decode("utf-8")
        .splitlines()
    )
...
def main():
    ...
    if platform_name.startswith("Windows"):
        dest_path = Path(os.getenv("APPDATA")).joinpath("Code").joinpath("User")
